...
- **オリジナル＋本番ディレクトリ** — `POST /cms/api/publish-lp.php` で `…/<site_key>/_lp_original`（凍結）と `…/_lp_publish`（本番用再生成・リドゥ可）。例は `server/docs/examples/lp-site-root.htaccess.example`。**全節の編集**は `content` スロット拡張と HTML 合意の段階的対応
